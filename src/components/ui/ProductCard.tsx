    ...
    <div className="bg-white rounded-[1vw] border border-slate-200/90 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group h-full relative overflow-hidden">
      ...
      <Link href={`/products/${product.slug}`} className="p-[1vw] space-y-[1vh] block cursor-pointer">
        ...
        <div className="flex items-start justify-between gap-[0.6vw]">
          <h3 className="text-[1.2vw] min-text-[16px] font-bold text-[#0B3C83] group-hover:text-blue-600 transition-colors tracking-tight line-clamp-1">
            {product.name}
          </h3>
        </div>
        ...
        {hideDetails ? (
          ...
          <div className="grid grid-cols-12 gap-[1.2vw] items-center justify-around pt-[0.4vh]">
            ...
            <div className="col-span-4 relative aspect-[4/5] w-full bg-slate-50/80 rounded-[0.8vw] border border-slate-200/90 p-[0.5vw] flex items-center justify-center overflow-hidden group-hover:border-blue-300 transition-colors shrink-0">
              <Image
                src={product.image}
                alt={product.name}
                fill
            ...
            <div className="col-span-8 flex  gap-[1vw] overflow-hidden ">
              {specEntries.length > 0 ? (
                <>
                  {/* Container 1: Keys / Labels Column */}
                  <div className="flex flex-col justify-around space-y-[1.5vh] shrink-0">
                    {specEntries.slice(0, 7).map(([key]) => (
                      <span key={key} className="font-extrabold text-slate-900 text-[0.93vw] min-text-[13px] leading-snug">
                        {key}:
                      </span>
                    ))} 
                  </div>
                  ...
                  <div className="flex flex-col justify-around space-y-[1.5vh] ml-[4vw] overflow-hidden flex-1 min-w-0">
                    {specEntries.slice(0, 7).map(([key, value]) => (
                      <span key={key} className="font-medium text-slate-700 text-[0.93vw] min-text-[13px] leading-snug truncate">
                        {value}
                      </span>
                    ))}
                  </div>
                ...
                <p className="text-slate-600 font-normal leading-relaxed line-clamp-4 text-[0.9vw]">
                  {product.description}
                </p>
              )}
            </div>
      ...
      <div className="bg-gradient-to-r from-blue-100/90 via-sky-100/70 to-sky-100/60 backdrop-blur-xl border-t border-blue-200/90 py-[1vh] px-[1vw] flex items-center justify-end gap-[0.8vw] mt-[0.3vh]">
        ...
        <Link href={`/contact?product=${encodeURIComponent(product.name)}`} className="w-full sm:w-auto">
          <button className="w-full sm:w-auto bg-[#0B3C83] hover:bg-[#062454] text-white text-[0.78vw] min-text-[11px] font-extrabold py-[0.8vh] px-[1.2vw] rounded-[0.6vw] transition-all duration-300 flex items-center justify-center gap-[0.4vw] shadow-md shadow-blue-900/10 active:scale-[0.98]">
            SEND INQUIRY
            <ArrowRight className="w-[0.85vw] h-[0.85vw] min-w-[12px] min-h-[12px] stroke-[2.5]" />
          </button>
        </Link>
      </div>
    </div>
